...
def calc_EHH_conditional_on_frequency(params, pop_name, distance_in_bp, nrep, data_dir, ehh_data_dir):
    ...
    t5 = params['demography_in_year'][4][1]
    t6 = params['demography_in_year'][5][1]
    ...
    N5 = params['demography_in_year'][5][2]
    N6 = params['demography_in_year'][6][2]

    max_age = int(5000 * params['generation'] * 16)
    mutation_age_candidate = np.arange(7000, max_age + 1, 1)
    # popsize range in year
    N0_ages = np.arange(7000, t1 + 1, 1)
    N1_ages = np.arange(t1 + 1, t2 + 1, 1)
    N2_ages = np.arange(t2 + 1, t3 + 1, 1)
    N3_ages = np.arange(t3 + 1, t4 + 1, 1)
    N4_ages = np.arange(t4 + 1, t5 + 1, 1)
    N5_ages = np.arange(t5 + 1, t6 + 1, 1)
    N6_ages = np.arange(t6 + 1, max_age + 1, 1)
    ...
    weight = [N0 / N0 for i in range(len(N0_ages))] + [N1 / N0 for i in range(len(N1_ages))] + \
             ...
             [N4 / N0 for i in range(len(N4_ages))] + [N5 / N0 for i in range(len(N5_ages))] + \
             [N6 / N0 for i in range(len(N6_ages))]
    ...
    for i in labels:
        EHH_data_dic[i].to_csv(
            '{}/EHH_data_f{}_s{}_popname{}_datanum{}.csv'
                .format(ehh_data_dir, i, params['s'], pop_name, params['data_num']))
    print(pop_name, 'data_num', params['data_num'], 'done', num_run, 'times run')


def calc_power(ehh_data_dir, data_num, pop_name, s, nrep, path_to_percentile, power_dir, file_name):
    ...
    for m in labels:
        for n in data_num:
            df = pd.read_csv(
                '{}/EHH_data_f{}_s{}_popname{}_datanum{}.csv'
                .format(ehh_data_dir, m, s, pop_name, n), index_col=0)
            EHH_data_dic[m] = pd.concat([EHH_data_dic[m], df], axis=0, ignore_index=True)
        #print(len(EHH_data_dic[m]))
        EHH_data_dic[m].to_csv('{}/EHH_data_f{}_s{}_popname{}.csv'
                               .format(ehh_data_dir, m, s, pop_name), index=False)
    ...
    for f in labels:
        ...
        df = pd.read_csv('{}/EHH_data_f{}_s{}_popname{} .csv'
                         .format(ehh_data_dir, f, s, pop_name))
        df = df[:nrep]
...
def main():
    ...
    calc_power(ehh_data_dir, data_num, pop_name = pop_name, s=sel_advantages[0],
