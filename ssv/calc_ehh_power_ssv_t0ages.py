...
def calc_stat(params, data_dir, ehh_data_dir, distance_in_bp):
    ...
    # run R to calc EHH
    mbslib.run_command('Rscript calc_EHH_given_t_p.R {} {} {} {} {} {} {} {}'
                       .format(params['t0_in_year'], params['p0'], params['n_trajectory'],
                               params['lsites'], params['s'], distance_in_bp, data_dir, ehh_data_dir))

    print(params["t0_in_year"], params['p0'], params["s"], "done")


def calc_power(t0_list, s, p, ehh_data_dir, path_to_percentile , power_dir, n_run):
    rEHH_power_list = []
    iHS_power_list = []
    # calc rEHH power
    rEHH_percentile = path_to_percentile[:32] + 'rEHH' + path_to_percentile[32:]
    ...
    for t in t0_list:
        df = pd.read_csv('{}/EHH_data_t0{}_p0{}_s{}.csv'.format(ehh_data_dir, t, p, s))
    ...
    rEHH_power_df = pd.DataFrame(rEHH_power_list, columns=p, index=t0_list)
    rEHH_power_df.to_csv("{}/rEHH_power_p0{}_s{}_t0ages.csv".format(power_dir, p, s))

    # iHS
    iHS_percentile = path_to_percentile[:32] + 'iHS' + path_to_percentile[32:]
    ...
    iHS_power_df = pd.DataFrame(iHS_power_list, columns=p, index=t0_list)
    iHS_power_df.to_csv("{}/iHS_power_p0{}_s{}_t0ages.csv".format(power_dir, p, s))
...
def main():
    ...
    calc_power(t0_ages, sel_advantages[0], initial_freq, ehh_data_dir, path_to_percentile, power_dir, nrep)
