...
def calc_EHH_conditional_on_frequency(params, distance_in_bp, data_dir, ehh_data_dir, nrep):
    ...
    for i in labels:
        EHH_data_dic[i].to_csv(
            '{}/EHH_data_f{}_s{}_bage{}_bduration{}_bstrength{}_datanum{}.csv'
...
def calc_power(ehh_data_dir, data_num, b_end, b_duration, k, s, nrep, path_to_percentile, power_dir, file_name):
    ...
    for m in labels:
        for n in data_num:
            df = pd.read_csv(
                '{}/EHH_data_f{}_s{}_bage{}_bduration{}_bstrength{}_datanum{}.csv'
                .format(ehh_data_dir, m, s, b_end, b_duration, k, n), index_col=0)
            EHH_data_dic[m] = pd.concat([EHH_data_dic[m], df], axis=0, ignore_index=True)
        #print(len(EHH_data_dic[m]))
        EHH_data_dic[m].to_csv('{}/EHH_data_f{}_s{}_bage{}_bduration{}_bstrength{}.csv'
                               .format(ehh_data_dir, m, s, b_end, b_duration, k), index=False)
    ...
    for f in labels:
        ...
        df = pd.read_csv('{}/EHH_data_f{}_s{}_bage{}_bduration{}_bstrength{} .csv'
                         .format(ehh_data_dir, f, s, b_end, b_duration, k))
        df = df[:nrep]
...
def main():
    ...
    calc_power(ehh_data_dir, data_num, b_end, b_duration, int(1 / k), sel_advantages[0], nrep, path_to_percentile, power_dir, file_name)
